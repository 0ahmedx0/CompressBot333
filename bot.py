...
def handle_video(client, message):
    file = client.download_media(
        ...
        progress=download_progress # إضافة دالة progress للتنزيل هنا
    )
    ...
    try:
        ...
        if callback_query.data == "crf_27": # ضغط عالي
            if message.animation:
                ...
                ffmpeg_command = f'ffmpeg -y -i "{file}" -r {VIDEO_FPS} -c:v {VIDEO_CODEC} -pix_fmt {VIDEO_PIXEL_FORMAT} -b:v {500k} -crf 27 -preset {ultrafast} -c:a {VIDEO_AUDIO_CODEC} -b:a {VIDEO_AUDIO_BITRATE} -ac {VIDEO_AUDIO_CHANNELS} -ar {VIDEO_AUDIO_SAMPLE_RATE} -profile:v {high} -map_metadata -1 "{temp_filename}"'
        elif callback_query.data == "crf_23": # ضغط متوسط
            if message.animation:
                ffmpeg_command = f'ffmpeg -y -i "{file}" "{temp_filename}"'
            else:
                ffmpeg_command = f'ffmpeg -y -i "{file}" -r {VIDEO_FPS} -c:v {VIDEO_CODEC} -pix_fmt {VIDEO_PIXEL_FORMAT} -b:v {1500k} -crf 23 -preset {medium} -c:a {VIDEO_AUDIO_CODEC} -b:a {VIDEO_AUDIO_BITRATE} -ac {VIDEO_AUDIO_CHANNELS} -ar {VIDEO_AUDIO_SAMPLE_RATE} -profile:v {high} -map_metadata -1 "{temp_filename}"'
        elif callback_query.data == "crf_18": # ضغط منخفض
            if message.animation:
                ffmpeg_command = f'ffmpeg -y -i "{file}" "{temp_filename}"'
            else:
                ffmpeg_command = f'ffmpeg -y -i "{file}" -r {VIDEO_FPS} -c:v {VIDEO_CODEC} -pix_fmt {VIDEO_PIXEL_FORMAT} -b:v {VIDEO_BITRATE} -crf 18 -preset {medium} -c:a {VIDEO_AUDIO_CODEC} -b:a {VIDEO_AUDIO_BITRATE} -ac {VIDEO_AUDIO_CHANNELS} -ar {VIDEO_AUDIO_SAMPLE_RATE} -profile:v {high} -map_metadata -1 "{temp_filename}"'
